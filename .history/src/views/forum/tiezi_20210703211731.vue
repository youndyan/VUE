...
<el-table-column label="用户名" prop="user.userName" />
 <el-table-column label="用户头像">
        <template slot-scope="scope">
          <img width="50px" :src="scope.row.user.avatar | dalImg" style="max-width:120px;max-height:120px;diaplay:block;magin:0 auto;" alt="" />
          </template>
          </el-table-column>
       <el-table-column label="内容" prop="name" />
<el-table-column label="帖子图片">
        <template slot-scope="scope">
          <img width="80px" :src="scope.row.coverImage | dalImg" style="max-width:120px;max-height:120px;diaplay:block;magin:0 auto;" alt="" />
          </template>
          </el-table-column>
...
<el-button class="btn" icon="el-icon-edit" size="small" type="primary" @click="editOne(scope.row)">评论管理</el-button>
...
<script>
import { loadTiezisApi, deleteOneApi } from '@/api/tiezi'
export default {
  name: 'TieziList',
  data() {
    return {
  ...
  methods: {
    async loadData(page = 1) {
      const data = await loadTiezisApi(page)
      console.log(data)
      this.list = data.data
